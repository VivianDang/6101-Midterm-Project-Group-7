...
logistic_1 = glm(Churn ~ SeniorCitizen + Dependents + tenure + MultipleLines + InternetService
                         + StreamingTV + StreamingMovies + Contract + PaperlessBilling + TotalCharges, 
                         data = customer, family = 'binomial')
summary(logistic_1)
...
#### Confusion matrix 
```{r Q7.confusionMatrix, results='markup'}
loadPkg("regclass")
confusion_matrix(logistic_1)
xkabledply( confusion_matrix(logistic_1), title = "Confusion matrix from Logit Model" )
unloadPkg("regclass")
```
```{r Q7.confusion matrix, results=T}
loadPkg("regclass")
cfmatrix1 = confusion_matrix(logistic_1)
...
```{r Q7.McFadden, results=T}
loadPkg("pscl")
ChurnLogitpr2 = pR2(logistic_1)
ChurnLogitpr2
unloadPkg("pscl") 
```
With the McFadden value of `r ChurnLogitpr2['McFadden']`, which is analogous to the coefficient of determination $R^2$, only about 27.4% of the variations in y is explained by the explanatory variables in the model. 